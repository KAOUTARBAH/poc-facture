...
void findById_should_return_facture() {
	//When
	Optional<Facture> facture = this.factureRepository.findById(1L);
...
void save_should_insert_new_facture() {
    ...
    newFacture.setCustomerRef("Libelle facture TEST");
    //newFacture.setMontant(2500F);
    newFacture.setCustomerContact("Client@Test.com");
    newFacture.setId(5L);
...
void save_should_update_existing_facture() {
	...
	existingFacture.setCustomerRef("FIRST Libelle");
	existingFacture.setCustomerContact("FIRST@Client.fr");
    ...
    assertEquals("FIRST Libelle", updatedFacture.getCustomerRef());
    assertEquals("FIRST Client", updatedFacture.getCustomerContact());
